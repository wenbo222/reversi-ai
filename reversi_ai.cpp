...
{
    int total=0;
    for(int r=0;r<8;r++) for(int c=0;c<8;c++) if(!pieces_to_flip(board, player, c, r).empty()) total++;
    return total;
}
...
static double evaluate(int board[8][8], int player)
{
    ...
    if(player!=E&&legal_moves_count(board, player)==0)
    {
        if(num_b>num_w) return 100000.0+num_adv;
        if(num_w>num_b) return -100000.0+num_adv;
        return 0.0;
    }
    int mob_adv=legal_moves_count(board, B)-legal_moves_count(board, W);
...
static bool load_bin(const string& path)
{
    ...
    // Read Zobrist hashes
    f.read((char*)g_zob_b, 512);
    f.read((char*)g_zob_w, 512);
    f.read((char*)&g_zob_player_b, 8);
    f.read((char*)&g_zob_player_w, 8);
